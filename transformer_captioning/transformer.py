...
class AttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.bmm(query, key.transpose(-2, -1)) / (self.embed_dim**0.5)
        ...
        if attn_mask is not None:
            ...
            additive_mask = - LARGE_VALUE * (1 - attn_mask.float())
...
class MultiHeadAttentionLayer(AttentionLayer):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        dot_product = torch.matmul(query, key.transpose(-2, -1)) / ((self.embed_dim // H)**0.5)
        ...
        output = torch.matmul(y, value).transpose(1, 2).reshape(N, S, -1)
...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        output = x + self.encoding(torch.arange(S, device = x.device).unsqueeze(0)).expand(N, S, -1)
...
class SelfAttentionBlock(nn.Module):
    ...
    def forward(self, seq, mask):
        ...
        output = self.self_attn(seq, seq, seq, attn_mask = mask) 
        output = self.dropout(output)
        output = self.layernorm(seq + output)
        return output
...
class TransformerDecoder(nn.Module):
    ...
    def get_causal_mask(self, _len):
        ...
        coord_x, coord_y = torch.meshgrid(torch.arange(_len, device = self.device), torch.arange(_len, device = self.device), indexing = 'ij')
        mask = (coord_x > coord_y).bool()
        return mask
